...
## Rebranding-Entscheidung (Nutzer, 21.09.2026)
Nutzerwunsch: bestehende Apps (u.a. Hebleo) als Gestaltungsvorbild heranziehen. Rechercheergebnis: Hebleo ist eine videobasierte Kursplattform ohne vergleichbares interaktives Übungs-GUI; ihr Löwen-Logo/Wortmarke wurde **nicht** übernommen — das wäre eine Markenrechtsverletzung unabhängig vom Kontext. Übernommen wurde ausschließlich der allgemeine visuelle Stil (kräftige Blautöne, Pillen-Buttons).

Der Nutzer hat sich nach mehreren Iterationsrunden explizit für folgende, DESIGN_SYSTEM.md überschreibende Änderungen entschieden (finale Freigabe: „das passt so und kann übernommen werden”):
1. **Farbpalette**: Wechsel von der ursprünglichen ruhigen Cremeton-/Petrol-Palette zu einer kräftigen Blau-Palette (neue Werte in DESIGN_SYSTEM.md „Starttokens”). Kontrast ist weiterhin vor Release per WCAG AA zu prüfen — das war bereits vor dem Rebranding offen und bleibt es.
2. **Maskottchen erlaubt, aber begrenzt**: Das ursprüngliche Komplettverbot von Maskottchen (v1.0/DESIGN_SYSTEM.md) wird für genau ein selbst entworfenes Maskottchen aufgehoben — ein Wiedehopf (דוכיפת, Israels Nationalvogel), bewusst kein Löwe und keine Kopie einer fremden Marke. Vier Ausdrücke (winkend, fliegend, fröhlich, gratulierend) wurden über mehrere Bildvorschau-Runden mit dem Nutzer abgestimmt und in `HebrewApp/DesignSystem/HoopoeMascot.swift` als natives SwiftUI-`Canvas`/`GraphicsContext`-Zeichnen umgesetzt (keine Bilddateien, kein SVG-Renderer). Weiterhin gilt: keine zusätzlichen Comicfiguren, kein Konfetti, keine Leaderboards, keine Streak-Drohungen.
3. **Platzierung** (technische Implementierungsentscheidung, reversibel, ohne gesonderte Nutzerfreigabe): winkend/fröhlich auf der „Heute”-Begrüßungskarte (je nachdem ob noch eine Lernstrecke offen ist), gratulierend auf dem Lernstrecken-Abschluss-Screen. Die fliegende Pose ist im Designsystem verfügbar, aber noch nirgends platziert — es gibt aktuell keinen Hero-/Splash-Screen, in den sie ohne zusätzlichen Scope passen würde.
